...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.groupBy(s => s.toLower).map(c => (c._1, c._2.length)).toList.sortBy(_._1)
  }
  ...
  loadDictionary map (w => w -> wordOccurrences(w)) groupBy (_._2) map (t => (t._1, t._2 map (t2 => t2._1)))

  /** Returns all the anagrams of a given word. */
  def wordAnagrams(word: Word): List[Word] =
  dictionaryByOccurrences.find(_._1 == wordOccurrences(word)) map (_._2) match {
    case Some(x) => x
    case None => Nil
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty)
      ...
      for {
        ...
        if (counter > 0) (letter, counter) :: rest
        else rest
      }
    }
  }
  ...
  def isSubset(x: Occurrences, y: Occurrences) = {
    val xs = (x toMap).withDefaultValue(0)
    (for { (letter, number) <- y
           if xs(letter) < number
    } yield 1).isEmpty
  }
